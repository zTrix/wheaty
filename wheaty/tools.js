...
var Helpers = {
    ...
    gravitar: function gravitar(email, size) {
        size = size || 200;
        return "http://www.gravatar.com/avatar/" + MD5.md5((email + "").trim().toLowerCase()) + "?r=pg&s=" + size + ".jpg&d=identicon";
    },
    formatDate: function formatDate(val, format) {
        ...
        while (match = format.match(/(%[a-z%])/i)) {
            ...
            case "%m":
                ...
                break;
            case "%F":
                // A full textual representation of a month like March
                value = monthsInYear[date.getMonth()];
                break;
            case "%d":
            ...
            case "%u":
                ...
                value = pad(date.getMilliseconds(), 3);
                break;
            case "%z":
                // time zone offset
                value = getTZOString(date.getTimezoneOffset());
                break;
            case "%%":
                // literal % sign
                value = "\0%\0";
